...
class ProxyMiddleware(object):
    ...
    def process_request(self, request, spider):
        proxy_dict, proxy = self.ip.get_random_valid_ip()
        print(proxy)
...
class RandomUserAgentMiddleware(object):
    def __init__(self, crawler):
        ...
        self.ua = UserAgent()
        self.per_proxy = True  #True为每个proxy分配一个useragent, False则同一个proxy每次请求的useragent都是随机的。
        self.ua_type = 'random'  #crawler.settings.get('RANDOM_UA_TYPE', 'random')
        self.proxy2ua = {}
...
class SinaweiboDownloaderMiddleware(object):
    # Not all methods need to be defined. If a method is not defined,
